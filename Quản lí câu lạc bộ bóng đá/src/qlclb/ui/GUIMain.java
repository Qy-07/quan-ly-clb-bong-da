...
public class GUIMain {
    ...
    private void createMainGUI() {
        ...
        tableModel = new DefaultTableModel(cols, 0) {
            public boolean isCellEditable(int r, int c) {
                ...
                if (c == 6) {
                    // Chỉnh sửa được nếu "Dự bị" và "Không thi đấu" là false (vậy "Đá chính" chỉ có thể được chọn nếu các ô khác không được chọn)
                    return !(Boolean) getValueAt(r, 7) && !(Boolean) getValueAt(r, 8);
                } else if (c == 7) {
                    // Chỉnh sửa được nếu "Đá chính" và "Không thi đấu" là false (vậy "Dự bị" chỉ có thể được chọn nếu các ô khác không được chọn)
                    return !(Boolean) getValueAt(r, 6) && !(Boolean) getValueAt(r, 8);
                } else if (c == 8) {
                    // Chỉnh sửa được nếu "Đá chính" và "Dự bị" là false (vậy "Không thi đấu" chỉ có thể được chọn nếu các ô khác không được chọn)
        ...
        // Bộ kết xuất tùy chỉnh cho các cột boolean để hiển thị hộp kiểm ở giữa, luôn hiển thị nhưng bị vô hiệu hóa nếu không chỉnh sửa được
            ...
            public void tableChanged(TableModelEvent e) {
                if (currentView == View.PLAYERS && e.getType() == TableModelEvent.UPDATE) {
                    ...
                    if (col == 6) { // Đá chính đã thay đổi
                        ...
                        if (daChinh != null) {
                            // Khi "Đá chính" được chọn, "Dự bị" và "Không thi đấu" phải được bỏ chọn
                            if (daChinh) {
                        ...
                        if (duBi != null) {
                            // Khi "Dự bị" được chọn, "Đá chính" và "Không thi đấu" phải được bỏ chọn
                            if (duBi) {
                        ...
                        if (khongThiDau != null) {
                            // Khi "Không thi đấu" được chọn, "Đá chính" và "Dự bị" phải được bỏ chọn
                            if (khongThiDau) {
                                tableModel.setValueAt(false, row, 6);
                                tableModel.setValueAt(false, row, 7);
                            }
        ...
        this.btns[4].addActionListener(e -> {
            if (currentView == View.PLAYERS) {
                ql.ghiFileCSV(DATA_FILE);
                JOptionPane.showMessageDialog(mainFrame, "Đã lưu vào " + DATA_FILE);
            } else if (currentView == View.ASSISTANTS) {
                saveTroLyList();
                JOptionPane.showMessageDialog(mainFrame, "Đã lưu vào troly.csv");
            }
        });
    ...
    private void refreshTable() {
        ...
        if (currentView == View.PLAYERS) {
            ...
            // Điều chỉnh độ rộng cột cho cầu thủ
        ...
        // Thiết lập mục sortCombo dựa trên chế độ xem
        if (currentView == View.PLAYERS) {
        ...
        // Thiết lập trạng thái kích hoạt cho các điều khiển
